...
- Original sender lives under `forward_origin.sender_user` (Bot API 7.0+) with legacy fallback fields `forward_from` (private user) or `forward_sender_name` (when source has hidden forwards) and `forward_from_chat` (when source is a channel post). The service must read original sender id from `forward_origin.sender_user.id` first, then fall back to `forward_from.id`. Messages that resolve to neither (anonymous / hidden forwards) are skipped with a counted reason.
