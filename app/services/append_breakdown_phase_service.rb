# Creates the phase the timeline expects next, or with `start` the zero round a new breakdown opens with.
...
class AppendBreakdownPhaseService
  ...
  def call
    ...
    append(step) unless step.nil?
  end

  def start
    append(GameBreakdown::Timeline.new(@breakdown).opening_phase)
  end
