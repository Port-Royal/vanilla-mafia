...
RSpec.describe "Judge::Breakdowns night editing" do
  ...
  describe "GET /judge/breakdowns/:id/edit" do
    ...
    it "renders the night outcome select" do
      ...
      expect(response.body).to include('name="breakdown_phase[night_outcome]"')
    end

    it "does not offer a killed seat until the night is a kill" do
      get edit_judge_breakdown_path(breakdown)
    ...
    context "when the night is a kill" do
      ...
      it "offers the killed seat" do
        get edit_judge_breakdown_path(breakdown)
        expect(response.body).to include('name="breakdown_phase[killed_seat]"')
      end
  ...
  describe "PATCH /judge/breakdowns/:breakdown_id/phases/:id" do
    ...
    it "records a kill" do
      patch judge_breakdown_phase_path(breakdown, first_night),
            params: { breakdown_phase: { night_outcome: "kill", killed_seat: "7" } }

      expect(first_night.reload).to have_attributes(night_outcome: "kill", killed_seat: 7)
    end

    it "records a miss" do
      patch judge_breakdown_phase_path(breakdown, first_night), params: { breakdown_phase: { night_outcome: "miss" } }
      expect(first_night.reload).to have_attributes(night_outcome: "miss", killed_seat: nil)
    end

    it "drops a stale killed seat when the outcome becomes a miss" do
      first_night.update!(night_outcome: "kill", killed_seat: 7)

      patch judge_breakdown_phase_path(breakdown, first_night),
            params: { breakdown_phase: { night_outcome: "miss", killed_seat: "7" } }

      expect(first_night.reload.killed_seat).to be_nil
    end

    it "clears the outcome when nothing is recorded" do
      first_night.update!(night_outcome: "kill", killed_seat: 7)

      patch judge_breakdown_phase_path(breakdown, first_night), params: { breakdown_phase: { night_outcome: "" } }
    ...
    it "records the shots" do
      patch judge_breakdown_phase_path(breakdown, first_night),
            params: { breakdown_phase: { night_outcome: "miss" }, shots: { "2" => "7", "3" => "none" } }
    ...
    it "records the checks" do
      patch judge_breakdown_phase_path(breakdown, first_night),
            params: { breakdown_phase: { night_outcome: "miss" }, checks: { "don_check" => "5", "sheriff_check" => "2" } }
    ...
    context "when the roles are hidden" do
      ...
      it "stores no night internals" do
        patch judge_breakdown_phase_path(breakdown, first_night),
              params: { breakdown_phase: { night_outcome: "miss" }, shots: { "2" => "7" } }
    ...
    it "does not reach a phase of another breakdown" do
      ...
      patch judge_breakdown_phase_path(other, first_night), params: { breakdown_phase: { night_outcome: "miss" } }
